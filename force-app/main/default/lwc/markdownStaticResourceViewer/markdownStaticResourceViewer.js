...
export default class MarkdownStaticResourceViewer extends LightningElement {
  ...
  async initialiseResources() {
    ...
    if (!this.resourceList.length) {
      this.error =
        "No static resource names provided. Set the resourceNames @api property (comma-separated list).";
      return;
    }
  ...
  async ensureLibrariesLoaded() {
    ...
    LIBS.loadingPromise = Promise.all([
      ...
      .catch((e) => {
        ...
        throw new Error(
          "Failed to load markdown libraries from static resource markdownLib."
        );
      });
  ...
  async loadCurrentDocument() {
    ...
    try {
      ...
      console.error(
        "Error loading markdown document for resource",
        resourceName,
        e.message
      );
  ...
  async fetchMarkdown(resourceName) {
    ...
    if (!response.ok) {
      throw new Error(
        `Failed to load markdown from ${url} (status ${response.status})`
      );
    }
  ...
  renderMarkdownToHtml(markdown) {
    ...
    throw new Error(
      "Markdown library (marked) is not available or has an unexpected shape."
    );
  }
  ...
  async loadSetupDocument() {
    ...
    try {
      ...
      console.warn(
        "Optional SETUP markdown document could not be loaded",
        e.message
      );
    }
  }
