...
Options:
  ...
  --duration <days>        Scratch org duration. Default: 1
...
ensure_agent_user() {
  ...
  echo "Verified Service Agent user and 3/3 required system permission sets."
}
...
if [[ -z "${scratch_username}" ]]; then
  echo "Scratch org creation did not return a username." >&2
  jq . <<< "${create_json}" >&2
  exit 1
fi

echo "Scratch org created: ${scratch_alias} (${scratch_username})."
